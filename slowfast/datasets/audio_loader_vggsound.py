...
def pack_audio(cfg, audio_record, temporal_sample_index, wav_input=False):
    ...
    if wav_input:
        wave = _extract_wav_feature(cfg, samples, int(start_idx), int(end_idx))
        return wave
    else:
...
def _extract_wav_feature(cfg, samples, start_idx, end_idx):
    if samples.shape[0] < int(round(cfg.AUDIO_DATA.SAMPLING_RATE * cfg.AUDIO_DATA.CLIP_SECS)):
        import librosa
        samples = librosa.util.fix_length(samples, size=int(round(cfg.AUDIO_DATA.SAMPLING_RATE * cfg.AUDIO_DATA.CLIP_SECS)), mode='edge')
        samples = samples.reshape(int(cfg.AUDIO_DATA.NUM_FRAMES), int(round(cfg.AUDIO_DATA.SAMPLING_RATE * cfg.AUDIO_DATA.CLIP_SECS)/cfg.AUDIO_DATA.NUM_FRAMES))
    else:
