...
from .epicsound_record import EpicSoundAudioRecord

from .spec_augment import combined_transforms
from . import utils as utils
from .audio_loader_epic_sound import pack_audio

logger = logging.get_logger(__name__)


@DATASET_REGISTRY.register()
class Epicsound(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        assert mode in [
            ...
            "test",
            "train+val"
        ], "Split '{}' not supported for EPIC-SOUND".format(mode)
        self.cfg = cfg
        self.mode = mode
        ...
        logger.info("Constructing EPIC-SOUND Audio {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        """
        if self.mode == "train":
            path_annotations_pickle = [os.path.join(self.cfg.EPICSOUND.ANNOTATIONS_DIR, self.cfg.EPICSOUND.TRAIN_LIST)]
        elif self.mode == "val":
            path_annotations_pickle = [os.path.join(self.cfg.EPICSOUND.ANNOTATIONS_DIR, self.cfg.EPICSOUND.VAL_LIST)]
        elif self.mode == "test":
            path_annotations_pickle = [os.path.join(self.cfg.EPICSOUND.ANNOTATIONS_DIR, self.cfg.EPICSOUND.TEST_LIST)]
        else:
            path_annotations_pickle = [os.path.join(self.cfg.EPICSOUND.ANNOTATIONS_DIR, file)
                                       for file in [self.cfg.EPICSOUND.TRAIN_LIST, self.cfg.EPICSOUND.VAL_LIST]]
        ...
        for file in path_annotations_pickle:
            for tup in pd.read_pickle(file).iterrows():
                for idx in range(self._num_clips):
                    self._audio_records.append(EpicSoundAudioRecord(tup))
                    self._temporal_idx.append(idx)
        assert (
                len(self._audio_records) > 0
        ), "Failed to load EPIC-SOUND split {} from {}".format(
            self.mode, path_annotations_pickle
        )
        logger.info(
            "Constructing epicsound dataloader (size: {}) from {}".format(
    ...
    def __getitem__(self, index):
        ...
        if self.audio_dataset is None:
            self.audio_dataset = h5py.File(self.cfg.EPICSOUND.AUDIO_DATA_FILE, 'r')
